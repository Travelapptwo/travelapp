    ...
    function getTree1($pid,$step,$flag,$db,$table){
        ...
        $this->str.="<ul>";
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $catname = $row["catname"];
                //$str=str_repeat($flag,$step);
                $this->str .= "<li> <span>{$catname}</span>  <a href='delCategory.php?id={$id}' style='display:inline-block;text-align:center;color: white;width: 40px;height: 22px;margin-left: 10px;border-radius: 5px;background: #037DCE'>删除</a> <a href='editCategory.php?id={$id}'style='background: white;border-radius: 5px;display:inline-block;text-align:center;width: 40px;height: 22px;margin-left: 10px;'>编辑</a>";
